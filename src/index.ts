...
export class Core {
  ...
  static start(loggerOpts: LoggerOptions = {}) {
    ...
    Core.db.serialize(() => {
      ...
      // Files table
      Core.db.run('CREATE TABLE IF NOT EXISTS "files" ( "id" INTEGER NOT NULL, "application_id" INTEGER NOT NULL, "user_id" INTEGER, "file_name" TEXT NOT NULL, "private" INTEGER NOT NULL DEFAULT 1, "disabled" INTEGER NOT NULL DEFAULT 0, "disable_reason" TEXT DEFAULT  "No reason ", PRIMARY KEY("application_id","id"))');

      // Invites table
      Core.db.run('CREATE TABLE IF NOT EXISTS "invites" ( "code" TEXT NOT NULL, "application_id" INTEGER NOT NULL, "created_by" INTEGER NOT NULL, "claimed_by" INTEGER, "expires_at" INTEGER DEFAULT 0, PRIMARY KEY("application_id","code"))');
      ...
      Core.db.run('REPLACE INTO applications (id, name) VALUES (-1, "SystemGlobal")');
    });
    //#endregion
